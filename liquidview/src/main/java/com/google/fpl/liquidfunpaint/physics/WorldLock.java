...
public class WorldLock {
    ...
    private boolean isBlockAccelerometer = false;
    ...
    public void setBlockAccelerometer(boolean blockAccelerometer) {
        isBlockAccelerometer = blockAccelerometer;
        if (!blockAccelerometer) {
            setGravity(mLastAccelerometerGravityX, mLastAccelerometerGravityY, true);
        }
    }

    public void setGravity(float gravityX, float gravityY, boolean fromAccelerometer) {
        if (fromAccelerometer && isBlockAccelerometer) {
            mLastAccelerometerGravityX = gravityX;
            mLastAccelerometerGravityY = gravityY;
            return;
        }
        lock();
        try {
